...
export const useRecomendationStore = defineStore("RecomendationStore", {
	...
	actions: {
		// async getSideGSI(){
		//   let gsiSideData = await axios.get(
		//     "http://localhost:5000/api/gsi/side",
		//     {
		//     }
		//   );
		//   console.log("gsiSideData " + gsiSideData.data);
		//   if (gsiSideData.data){
		//     if (gsiSideData.data=="radiant"){
		//       console.log("gsiSideData radiant? " + gsiSideData.data);
		//       this.radiantSide = true;
		//     }
		//     else{
		//       console.log("gsiSideData dire? " + gsiSideData.data);
		//       this.radiantSide = false;
		//     }
		//   }
		// },
		async getDraftOCR() {
				...
				if (this.radiantSide) {
					if (heroNumber < 6) {
						if (ocrDraftData.data > 0) {
							this.allHeroes.allyHeroes.splice(
								heroNumber - 1,
								1,
								ocrDraftData.data
							);
						...
						if (ocrDraftData.data > 0) {
							this.allHeroes.enemyHeroes.splice(
								heroNumber - 6,
								1,
								ocrDraftData.data
							);
		...
		putHeroesId(heroNumber, heroID, side) {
			if (side == "enemy")
				this.allHeroes.enemyHeroes.splice(heroNumber - 1, 1, heroID);
			else if (side == "ally")
				this.allHeroes.allyHeroes.splice(heroNumber - 1, 1, heroID);
		...
		clearHeroesIdAll(side) {
			...
			console.log(
				"clearHeroesIdAll allHeroes " + JSON.stringify(this.allHeroes)
			);
		},
		clearAllAllyState() {
			setTimeout(() => {
				console.log(
					"allyHero1= " +
						JSON.stringify(this.allyHero1.heroStats.matchUp[0].with[0])
				);
		...
		getAllRecomendation() {
			console.log("getAllRecomendation");
			this.getAlly1(this.allHeroes.allyHeroes[0]);
			this.getAlly2(this.allHeroes.allyHeroes[1]);
			this.getAlly3(this.allHeroes.allyHeroes[2]);
			this.getAlly4(this.allHeroes.allyHeroes[3]);
			this.getAlly5(this.allHeroes.allyHeroes[4]);

			this.getVersus1(this.allHeroes.enemyHeroes[0]);
			this.getVersus2(this.allHeroes.enemyHeroes[1]);
			this.getVersus3(this.allHeroes.enemyHeroes[2]);
			this.getVersus4(this.allHeroes.enemyHeroes[3]);
			this.getVersus5(this.allHeroes.enemyHeroes[4]);
			// for(let i =0; i < 10; i++){
			//   `getAlly${i+1}`(this.allHeroes.allyHeroes[i]);
			//   `getVersus${i+1}`(this.allHeroes.enemyHeroes[i]);
			// }
		},
		getAlly1(id) {
			...
			if (id > 0) {
				...
				const { result: resultAlly1, variables: variablesAlly1 } = useQuery(
					VERSUS_QUERY,
					{}
				);
		...
		getAlly2(id) {
			...
			if (id > 0) {
				...
				const { result: resultAlly2, variables: variablesAlly2 } = useQuery(
					VERSUS_QUERY,
					{}
				);
		...
		getAlly3(id) {
			...
			if (id > 0) {
				...
				const { result: resultAlly3, variables: variablesAlly3 } = useQuery(
					VERSUS_QUERY,
					{}
				);
		...
		getAlly4(id) {
			...
			if (id > 0) {
				...
				const { result: resultAlly4, variables: variablesAlly4 } = useQuery(
					VERSUS_QUERY,
					{}
				);
		...
		getAlly5(id) {
			...
			if (id > 0) {
				...
				const { result: resultAlly5, variables: variablesAlly5 } = useQuery(
					VERSUS_QUERY,
					{}
				);
		...
		getVersus1(id) {
			...
			if (id > 0) {
				...
				let week = parseInt(new Date().getTime() / 1000);
				console.log("week= " + parseInt(new Date().getTime() / 1000));
				const { result: resultVersus1, variables: variablesVersus1 } = useQuery(
					VERSUS_QUERY,
					{}
				);
		...
		getVersus2(id) {
			if (id > 0) {
				console.log("getVersus id= " + id);
				let week = parseInt(new Date().getTime() / 1000);
				const { result: resultVersus2, variables: variablesVersus2 } = useQuery(
					VERSUS_QUERY,
					{}
				);
				...
				setTimeout(() => {
					...
					console.log(
						"resultVersus2 " +
							JSON.stringify(this.versusHero2.heroStats.matchUp[0].vs[0])
					);
				}, "2000");
			}
		},
		getVersus3(id) {
			...
			if (id > 0) {
				let week = parseInt(new Date().getTime() / 1000);
				const { result: resultVersus3, variables: variablesVersus3 } = useQuery(
					VERSUS_QUERY,
					{}
				);
				...
				setTimeout(() => {
					...
					console.log(
						"resultVersus3 " +
							JSON.stringify(this.versusHero3.heroStats.matchUp[0].vs[0])
					);
				}, "2000");
			}
		},
		getVersus4(id) {
			...
			if (id > 0) {
				let week = parseInt(new Date().getTime() / 1000);
				const { result: resultVersus4, variables: variablesVersus4 } = useQuery(
					VERSUS_QUERY,
					{}
				);
				...
				setTimeout(() => {
					...
					console.log(
						"resultVersus4 " +
							JSON.stringify(this.versusHero4.heroStats.matchUp[0].vs[0])
					);
				}, "2000");
			}
		},
		getVersus5(id) {
			...
			if (id > 0) {
				let week = parseInt(new Date().getTime() / 1000);
				const { result: resultVersus5, variables: variablesVersus5 } = useQuery(
					VERSUS_QUERY,
					{}
				);
				...
				setTimeout(() => {
					...
					console.log(
						"resultVersus5 " +
							JSON.stringify(this.versusHero5.heroStats.matchUp[0].vs[0])
					);
				}, "2000");
			}
		},
